...
@router.get("/{service_id}/code", response_model=ServiceCodeResponse)
async def get_code(
    service_id: int,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_current_user),
...
@router.put("/{service_id}/code", response_model=ApiResponse)
async def save_code(
...
@router.post("/{service_id}/code/validate", response_model=CodeValidationResponse)
